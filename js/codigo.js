...
botonEmpezar.addEventListener( "click", () => {
    ...
    const horas = document.getElementById('horas');
        ...
        let tiempoLimite = (Number(inputHoras.value) * 3600 + Number(inputMinutos.value) * 60 + Number(inputSegundos.value)) * 1000 + new Date().getTime();
        
        const idTemporizador = setInterval( () => {
          let objetoTiempo = getRemainingTime(tiempoLimite);
          inputHoras.value = objetoTiempo.remainHours;
          inputMinutos.value = objetoTiempo.remainMinutes;
          inputSegundos.value = objetoTiempo.remainSeconds;
      
          if(objetoTiempo.remainTime <= 1) {
    ...
    function getRemainingTime(tiempoLimite){
        let now = new Date(),
            remainTime = ((tiempoLimite - now) + 1000) / 1000;
